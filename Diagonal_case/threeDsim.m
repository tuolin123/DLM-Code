...
tic
locmaxZ = simulateLocMax(D,rho,sigma,niter);
toc
...
p=zeros(nz,nrho);
nnb=ones(nz,3)*2;
q=ddlm(z,ones(nz,3)*rho,nnb,3);
qnormalize=q./(sum(q)*dz);

%% %% Plot the continuous, discrete and simulation density together 
...
locmaxZ2 = [];
path_data = 'isoL505030nsim1000n1_gauss_stddev1.3.mat';
load( path_data );
...
load('presmooth_lookup_3D.mat')
load('lookup_3D_CDF.mat')
dp = cdf_table3D.p;
dx = cdf_table3D.x;
drho = cdf_table3D.rho;
dp(dp<0) = 0;
...
line(pval_ref,pval_disc, 'Color', 'g', 'LineWidth', 2);
hline = refline(1,0);
set(hline,'LineStyle',':');
