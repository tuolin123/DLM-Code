...
niter = round(nx./(14867-14766.*rho).*max_iter); % coefficient from linear regression of rho and sample iteration numbers
...
for iter = 1:length(rho)
    ...
    locmaxZ = simulateLocMax_Discrete(D,rho(iter),sigma,niter(iter));
    [fx,x] = ecdf(locmaxZ); 
...
rho = [0.45 0.86 0.94 0.97 0.98 0.99];
numlocmax = [8170 2396 974 529 314 180];
