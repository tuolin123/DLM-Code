...
niter = round(nx./(22599-22495.*rho).*max_iter); % coefficient from linear regression of rho and sample iteration numbers
...
for iter = 1:length(rho)
    ...
    locmaxZ = simulateLocMax(D,rho(iter),sigma,niter(iter),nondiag);
    [fx,x] = ecdf(locmaxZ); 
...
save('presmooth_lookup_2D_nondiag.mat', 'matcdf');
...
save('lookup_2D_CDF_nondiag.mat','cdf_table2D')
